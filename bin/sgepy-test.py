...
def main(args):
    ...
    if 'lambda' in args.test or 'all' in args.test:
        ...
        w = sgepy.Worker(tmp_dir=args.tmp_dir, verbose=True)
        ret = w(func, 2)
    ...
    if 'mem' in args.test or 'all' in args.test:
        ...
        w = sgepy.Worker(tmp_dir=args.tmp_dir, verbose=True, mem=mem)
        ret = w(func, 2)
    ...
    if 'time' in args.test or 'all' in args.test:
        ...
        w = sgepy.Worker(tmp_dir=args.tmp_dir, pkgs=pkgs, verbose=True, time=time)
        ret = w(func1, 1)
        assert ret == 2, 'dynamic time test failed'
    if 'error' in args.test or 'all' in args.test:
        logging.info('-- error function test --')
        func = lambda x: x ** 'x'
        w = sgepy.Worker(tmp_dir=args.tmp_dir, verbose=True, max_attempts=2)
        try:
            ret = w(func, 2)
        except ValueError:
            ret = None
    ...
    if 'pool' in args.test or 'all' in args.test:
        ...
        kwargs = {'y' : 2, 'z' : 2}
        pkgs = ['time']
        p = sgepy.Pool(tmp_dir=args.tmp_dir, kwargs=kwargs, pkgs=pkgs, n_jobs=2, verbose=False)
